...
BOOL AddressBookForm_OnNotify(HWND hWnd, WPARAM wParam, LPARAM lParam) {
	BOOL ret = 0;
	switch (LOWORD(wParam)) {
	case IDC_LIST_PERSONALS: ret = AddressBookForm_OnListViewItemClicked(hWnd, wParam, lParam); break;
...
BOOL AddressBookForm_OnListViewItemClicked(HWND hWnd, WPARAM wParam, LPARAM lParam) {
	TCHAR name[11];
	TCHAR address[64];
	TCHAR phoneNumber[12];
	TCHAR emailAddress[64];
